...
public class EclipseWorld extends World implements Renderable, Updatable {
	...
	public void add(Object object, RenderLayer layer) {
		...
		if (object instanceof Platform)
			addPlatform((Platform) object);
		if (object instanceof Renderable)
			addRenderable((Renderable) object, layer);
		if (object instanceof Updatable)
			addUpdatable((Updatable) object);
		if (object instanceof Useable)
			addUseable((Useable) object);

		if (object instanceof SelfInitializer)
			((SelfInitializer) object).worldInit(this);
	}
	...
	public void remove(Object object) {
		if (object instanceof Body)
			removeBody((Body) object);
		platforms.remove(object);
		for (RenderLayer layer : RenderLayer.values())
			renderLayers.get(layer).remove(object);
	...
	/**
	 * Returns what Platform, if any, the position is resting on, or null if
	 * none.
	 */
	public Platform standingOn(Vector2 position) {
		for (Platform platform : platforms) {
			if (platform.standingOn(position))
				return platform;
		}
		return null;
	}
